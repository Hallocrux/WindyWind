...
TRY_NAME = "087_joint_reg_ssl_condition_embedding"
...
class JointTinyTCN(nn.Module):
    def __init__(self, in_channels: int, embedding_dim: int) -> None:
        ...
        pooled_dim = 32 * self.pool.multiplier
    ...
    def encode(self, x: torch.Tensor) -> torch.Tensor:
        embedding = self.projector(self.pooled_features(x))
        return nn.functional.normalize(embedding, dim=1)

    def predict(self, x: torch.Tensor) -> torch.Tensor:
        return self.reg_head(self.pooled_features(x)).squeeze(-1)

    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        pooled = self.pooled_features(x)
        pred = self.reg_head(pooled).squeeze(-1)
        embedding = nn.functional.normalize(self.projector(pooled), dim=1)
        return pred, embedding


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Joint wind regression + structural contrastive embedding quickcheck.")
...
def main() -> None:
    ...
    write_summary(output_dir / "summary.md", summary_df, pred_df, knn_df, config)

    best = summary_df.iloc[0]
    print("087 joint reg ssl condition embedding quickcheck finished.")
    print(f"output_dir={output_dir}")
    print(f"lambda_ssl={config.lambda_ssl}")
...
def load_or_train_window_embeddings(
    *,
    ...
    model = JointTinyTCN(in_channels=X_train.shape[1], embedding_dim=config.embedding_dim)
    suffix = f"{window_label}_lambda{config.lambda_ssl:g}".replace(".", "p")
    ckpt_path = ckpt_dir / f"joint_{suffix}.pt"
    norm_path = ckpt_dir / f"joint_{suffix}_norm.npz"
    meta_path = ckpt_dir / f"joint_{suffix}.json"
    if ckpt_path.exists() and norm_path.exists() and meta_path.exists() and not force_retrain:
        ...
        set_seed(seed)
        train_joint_model(model, X_train_norm, y_train, train_case_ids, config)
            ...
            json.dumps(
                {
                    ...
                    "objective": "wind_mse_plus_lambda_case_supervised_contrastive",
...
def train_joint_model(model: nn.Module, X_train: np.ndarray, y_train: np.ndarray, case_ids: np.ndarray, config: TrainConfig) -> None:
    loader = DataLoader(
        TensorDataset(torch.from_numpy(X_train.copy()).float(), torch.from_numpy(y_train.copy()).float(), torch.from_numpy(case_ids.copy()).long()),
    ...
    for _ in range(config.max_epochs):
        for batch_x, batch_y, batch_case_ids in loader:
            reg_pred, _ = model(batch_x)
            reg_loss = reg_loss_fn(reg_pred, batch_y)
            ...
            ssl_embedding = model.encode(both_x)
            ssl_loss = supervised_contrastive_loss(ssl_embedding, both_case_ids, config.temperature)
            loss = reg_loss + config.lambda_ssl * ssl_loss
...
def build_embedding_case_table(record_df: pd.DataFrame, per_window: dict[str, dict[str, object]]) -> pd.DataFrame:
    ...
    result["embedding_concat"] = result.apply(
        lambda row: np.concatenate([np.asarray(row["embedding_2s"], dtype=float), np.asarray(row["embedding_8s"], dtype=float)]).astype(float),
        axis=1,
    )
    matrix = np.vstack(result["embedding_concat"].to_numpy())
    embedding_df = pd.DataFrame(matrix, columns=[f"embedding_{index + 1}" for index in range(matrix.shape[1])], index=result.index)
    return pd.concat([result.drop(columns=["embedding_2s", "embedding_8s", "embedding_concat"]), embedding_df], axis=1).reset_index(drop=True)
...
def run_added_to_added2(embedding_case_df: pd.DataFrame, embedding_columns: list[str]) -> pd.DataFrame:
    ...
    pred_map = {
        "rpm_knn4": base_pred,
        "joint_embedding_ridge": fit_predict_embedding_ridge(train_df, test_df, embedding_columns, "wind_speed"),
        "rpm_knn4_plus_joint_embedding_residual_ridge": base_pred + fit_predict_embedding_ridge(train_df, test_df, embedding_columns, residual_targets),
    }
    rows: list[dict[str, object]] = []
...
def write_summary(output_path: Path, summary_df: pd.DataFrame, pred_df: pd.DataFrame, knn_df: pd.DataFrame, config: TrainConfig) -> None:
    lines = [
        "# joint reg ssl condition embedding quickcheck",
        "",
        "- 状态：`current`",
        "- 首次确认：`2026-04-13`",
        "- 最近复核：`2026-04-13`",
        f"- `lambda_ssl = {config.lambda_ssl}`",
