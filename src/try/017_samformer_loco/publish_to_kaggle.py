...
ENTRY_REL_PATH = "src/try/017_samformer_loco/run_samformer_loco.py"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="发布 SAMformer windywind 实验到 Kaggle。")
    add_publish_args(
        parser,
        default_code_slug="windywind-samformer-code",
        default_kernel_slug="windywind-samformer-runner",
    )
...
def main() -> None:
    ...
    run_dir = make_run_dir(TRY_NAME, "samformer")
    ...
    write_dataset_metadata(
        code_dir,
        code_dataset_id,
        "windywind SAMformer code payload",
    )
    kernel_dir = prepare_kernel_dir(
        ...
        kernel_id=kernel_id,
        kernel_title="windywind SAMformer runner",
            ...
            "datasets": [
                {
                    ...
                    "dataset_title": "windywind SAMformer code payload",
                    "version_notes": "SAMformer code payload update",
    ...
    if not args.skip_remote:
        ensure_dataset_created(
            code_dataset_id,
            code_dir,
            run_dir / "code_dataset_creation.log",
        )
        run_publish_flow(
    ...
    print(f"SAMformer Kaggle 发布目录已准备：{run_dir}")
