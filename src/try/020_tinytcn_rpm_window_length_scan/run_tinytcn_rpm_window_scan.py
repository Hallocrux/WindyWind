...
TRY020_ROOT = REPO_ROOT / "src" / "try" / "020_tinytcn_rpm_window_length_scan"
for path in (REPO_ROOT, TRY009_ROOT, TRY012_ROOT, TRY019_ROOT, TRY020_ROOT):
...
TRY_NAME = "020_tinytcn_rpm_window_length_scan"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="运行 TinyTCN 转速回归窗长扫描。")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=OUTPUT_ROOT,
        help="输出目录，默认写到 outputs/try/020_tinytcn_rpm_window_length_scan。",
    )
    parser.add_argument(
        "--mode",
        choices=["dev", "full"],
...
def main() -> None:
    ...
    unlabeled_frames: list[pd.DataFrame] = []
    for window_label, window_config in WINDOW_CONFIGS:
        ...
        summary_row, case_df = summarize_rpm_predictions(prediction_frame, f"TinyTCN@{window_label}")
        ...
        if not unlabeled_df.empty:
            ...
            unlabeled_frames.append(unlabeled_df)
    ...
    unlabeled_df = (
        pd.concat(unlabeled_frames, ignore_index=True)
        if unlabeled_frames
        else pd.DataFrame(columns=["case_id", "file_name", "predicted_rpm", "window_label"])
    )

    summary_df.to_csv(output_dir / "rpm_window_scan_summary.csv", index=False, encoding="utf-8-sig")
    case_df.to_csv(output_dir / "rpm_window_scan_case_level_predictions.csv", index=False, encoding="utf-8-sig")
    unlabeled_df.to_csv(output_dir / "rpm_window_scan_unlabeled_predictions.csv", index=False, encoding="utf-8-sig")
    write_summary_markdown(output_dir, summary_df, records, unlabeled_df)

    print("TinyTCN 转速回归窗长扫描已完成。")
