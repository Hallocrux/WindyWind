...
TRY_NAME = "027_dualstream_tinytcn_quickcheck"
...
class DualStreamTinyTCN(nn.Module):
    def __init__(self, strain_idx: list[int], acc_idx: list[int]) -> None:
        super().__init__()
        self.register_buffer("strain_idx", torch.tensor(strain_idx, dtype=torch.long))
        self.register_buffer("acc_idx", torch.tensor(acc_idx, dtype=torch.long))
        self.strain_encoder = TinyTCNEncoder(in_channels=len(strain_idx), out_channels=16)
        self.acc_encoder = TinyTCNEncoder(in_channels=len(acc_idx), out_channels=32)
        self.head = nn.Sequential(
            nn.Linear(48, 24),
            nn.ReLU(),
            nn.Linear(24, 1),
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        strain_x = x.index_select(dim=1, index=self.strain_idx)
        acc_x = x.index_select(dim=1, index=self.acc_idx)
        strain_feat = self.strain_encoder(strain_x)
        acc_feat = self.acc_encoder(acc_x)
        feat = torch.cat([strain_feat, acc_feat], dim=1)
        return self.head(feat).squeeze(-1)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="快速验证双流 TinyTCN。")
...
def main() -> None:
    ...
    strain_idx, acc_idx = split_channel_indices(common_signal_columns)
    ...
    dual_better = is_dual_stream_positive(target_summary_df, target_case_df)
    if dual_better:
    ...
    write_summary_markdown(
        ...
        strain_count=len(strain_idx),
        acc_count=len(acc_idx),
    )

    print("双流 TinyTCN 快速验证已完成。")
    print(f"输出目录: {output_dir}")
    print(f"目标工况: {TARGET_CASE_IDS}")
    print(f"是否继续了对照工况: {dual_better}")


def split_channel_indices(common_signal_columns: list[str]) -> tuple[list[int], list[int]]:
    strain_idx = [i for i, col in enumerate(common_signal_columns) if "应变" in col]
    acc_idx = [i for i, col in enumerate(common_signal_columns) if "Acc" in col]
    if not strain_idx or not acc_idx:
        raise ValueError("未能正确识别应变与加速度通道。")
    return strain_idx, acc_idx
...
def evaluate_case_group(
    ...
    for case_id in case_ids:
        ...
        for variant_name in ("SingleStreamTinyTCN", "DualStreamTinyTCN"):
            ...
            if variant_name == "SingleStreamTinyTCN":
                ...
                model = DualStreamTinyTCN(strain_idx=strain_idx, acc_idx=acc_idx).to(device)
            train_model(
                model=model,
...
def is_dual_stream_positive(summary_df: pd.DataFrame, case_df: pd.DataFrame) -> bool:
    if summary_df.empty:
        return False
    dual_mae = float(summary_df.loc[summary_df["variant_name"] == "DualStreamTinyTCN", "case_mae"].iloc[0])
    single_mae = float(summary_df.loc[summary_df["variant_name"] == "SingleStreamTinyTCN", "case_mae"].iloc[0])
    base = case_df[case_df["variant_name"] == "SingleStreamTinyTCN"][["case_id", "abs_error"]].rename(columns={"abs_error": "single_abs_error"})
    dual = case_df[case_df["variant_name"] == "DualStreamTinyTCN"][["case_id", "abs_error"]].rename(columns={"abs_error": "dual_abs_error"})
    merged = dual.merge(base, on="case_id", how="left")
    improved_count = int((merged["dual_abs_error"] < merged["single_abs_error"]).sum())
    return dual_mae < single_mae and improved_count >= 3
...
def write_summary_markdown(
    ...
    lines = [
        ...
        f"- 通道拆分：`strain={strain_count}`，`acc={acc_count}`",
        "- 窗口口径：`50Hz / 5s / 2.5s`",
        "",
        "## 目标难工况汇总",
        "",
    ...
    lines.extend(["## 对照工况汇总", ""])
    if dual_better and not control_summary_df.empty:
    ...
    lines.extend(
        [
            ...
            "- 这里的双流只改变通道组织方式，不改变窗长与评估口径。",
            "- 如果双流在目标难工况和对照工况上都给出稳定正信号，才值得继续进入更大范围复核。",
        ]
    )
