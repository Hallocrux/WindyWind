...
TRY_NAME = "034_added_validation_label_check"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="用 added 数据检查可疑标签。")
...
def main() -> None:
    ...
    clean_train_records = [
        ...
        if record.is_labeled and record.case_id not in set(HARD_CASE_IDS)
    ]
    suspicious_records = [record for record in final_records if record.case_id in set(HARD_CASE_IDS)]

    prediction_frames = {}
    for label, window_config in WINDOW_CONFIGS.items():
        prediction_frames[label] = train_and_predict_external(
            train_records=clean_train_records,
            eval_records=[*added_records, *suspicious_records],
    ...
    combined_df = merge_window_predictions(prediction_frames)
    added_df = combined_df[combined_df["case_id"].between(21, 24)].copy()
    suspicious_df = combined_df[combined_df["case_id"].isin(HARD_CASE_IDS)].copy()
    summary_df = build_summary(added_df, suspicious_df)
    ...
    write_summary_markdown(output_dir / "summary.md", added_df, suspicious_df, summary_df)

    print("added 外部验证与可疑标签检查已完成。")
...
def merge_window_predictions(prediction_frames: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    for label, df in prediction_frames.items():
        rename_map = {"pred_wind_speed": f"pred_{label}"}
        block = df.rename(columns=rename_map)
        if base is None:
            base = block
        else:
...
def build_summary(added_df: pd.DataFrame, suspicious_df: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for name, df in (("added_cases", added_df), ("suspicious_cases", suspicious_df)):
        for model_name in ["2s", "5s", "8s", "2s_8s_fusion"]:
            rows.append(
                {
                    "group_name": name,
                    "model_name": model_name,
                    "case_mae": float(df[f"abs_error_{model_name}"].mean()),
                    "case_rmse": float(np.sqrt(np.mean(np.square(df[f"signed_error_{model_name}"])))),
...
def write_summary_markdown(output_path: Path, added_df: pd.DataFrame, suspicious_df: pd.DataFrame, summary_df: pd.DataFrame) -> None:
    lines = [
        "# added 外部验证与可疑标签检查",
        "",
        f"- 去掉的难工况：`{HARD_CASE_IDS}`",
        "- 训练池：`final` 带标签工况去掉难工况",
    ...
    for _, row in suspicious_df.iterrows():
        lines.append(
            f"- `工况{int(row['case_id'])}`: true=`{row['true_wind_speed']:.4f}`, pred_5s=`{row['pred_5s']:.4f}`, pred_fusion=`{row['pred_2s_8s_fusion']:.4f}`, signed_error_fusion=`{row['signed_error_2s_8s_fusion']:+.4f}`"
        )
    lines.extend(["", "## 汇总", ""])
    for _, row in summary_df.iterrows():
        lines.append(
            f"- `{row['group_name']} | {row['model_name']}`: case_mae=`{row['case_mae']:.4f}`, case_rmse=`{row['case_rmse']:.4f}`"
        )
