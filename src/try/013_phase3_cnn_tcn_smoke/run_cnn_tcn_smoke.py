...
TRY_NAME = "013_phase3_cnn_tcn_smoke"
OUTPUT_ROOT = REPO_ROOT / "outputs" / "try" / TRY_NAME
DEV_CASE_IDS = [1, 2, 3, 5, 15, 16]
MODEL_NAMES = ["TabularReference_G6_Ridge", "Tiny1DCNN", "TinyTCN"]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="运行第三阶段 CNN / TCN smoke。")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=OUTPUT_ROOT,
        help="输出目录，默认写到 outputs/try/013_phase3_cnn_tcn_smoke。",
    )
    parser.add_argument(
        "--mode",
        choices=["dev", "full"],
...
def main() -> None:
    ...
    for model_name in MODEL_NAMES:
        if model_name == "TabularReference_G6_Ridge":
            ...
            prediction_frame = evaluate_torch_model_loco(raw_dataset, model_name, train_config)
    ...
    summary_df.to_csv(output_dir / "cnn_tcn_model_summary.csv", index=False, encoding="utf-8-sig")
    case_df.to_csv(output_dir / "cnn_tcn_case_level_predictions.csv", index=False, encoding="utf-8-sig")
    write_summary_markdown(output_dir, summary_df, records)

    print("cnn/tcn smoke 已完成。")
