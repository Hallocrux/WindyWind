...
from phase3_end_to_end_lib import RawDataset, build_raw_window_dataset, evaluate_tabular_reference_loco
...
def evaluate_torch_model_loco(
    ...
    for _, (train_idx, valid_idx) in split_map.items():
        ...
        model = build_torch_model(model_name, in_channels=X_train.shape[1]).to(device)
        train_torch_model(
            model=model,
...
def predict_torch_model_unlabeled(
    dataset: RawDataset,
    model_name: str,
    train_config: TorchTrainConfig,
    random_seed: int = 42,
...
def build_torch_model(model_name: str, in_channels: int) -> nn.Module:
    if model_name == "Tiny1DCNN":
        return Tiny1DCNN(in_channels=in_channels)
    if model_name == "TinyTCN":
        return TinyTCN(in_channels=in_channels)
