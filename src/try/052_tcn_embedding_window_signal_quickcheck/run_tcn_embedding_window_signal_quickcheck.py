...
TRY_NAME = "052_tcn_embedding_window_signal_quickcheck"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="TinyTCN embedding 窗长信号 quickcheck。")
...
def main() -> None:
    ...
    domain_by_case_id = {
        **{record.case_id: "final_focus" for record in holdout_records if record.case_id < 21},
        **{record.case_id: "added_focus" for record in holdout_records if record.case_id >= 21},
    }
    ...
    print("052 TinyTCN embedding window signal quickcheck 已完成。")
...
def run_holdout_fold(
    *,
    ...
    for order, (window_label, window_config) in enumerate(WINDOW_CONFIGS.items(), start=1):
        ...
        per_window_result[window_label] = {
            "train_case_df": add_rpm_oof_predictions(train_case_df),
            "eval_case_df": eval_case_df,
        }

    base_pred = predict_rpm_knn4(train_records, holdout.rpm)
    case_rows = [build_case_row(domain_name, "rpm_knn4", holdout, base_pred)]
    neighbor_rows: list[dict[str, object]] = []

    for window_label in ("2s", "5s", "8s"):
        eval_case_df = per_window_result[window_label]["eval_case_df"]
        case_rows.append(
            build_case_row(
                domain_name,
                f"tinytcn_direct_{window_label}",
                holdout,
                float(eval_case_df["pred_direct"].iloc[0]),
            )
        )
        residual_pred, residual_neighbors = predict_embedding_knn(
            train_case_df=per_window_result[window_label]["train_case_df"],
            eval_embedding=np.asarray(eval_case_df["embedding"].iloc[0], dtype=float),
            target_column="rpm_residual_oof",
            neighbor_tag=f"embedding_residual_knn4_{window_label}",
            holdout_case_id=holdout.case_id,
        )
        residual_clip = max(
            float(per_window_result[window_label]["train_case_df"]["rpm_residual_oof"].abs().quantile(0.95)),
            0.15,
        )
        ...
        case_rows.append(
            build_case_row(
                domain_name,
                f"rpm_knn4__plus__embedding_residual_knn4_{window_label}_w0.5",
                holdout,
                base_pred + 0.5 * residual_pred,
            )
        )
        for row in residual_neighbors:
            row["holdout_domain"] = domain_name
            neighbor_rows.append(row)

    pred_2s = float(per_window_result["2s"]["eval_case_df"]["pred_direct"].iloc[0])
    pred_8s = float(per_window_result["8s"]["eval_case_df"]["pred_direct"].iloc[0])
    case_rows.append(
        build_case_row(
            domain_name,
            "tinytcn_direct_2s_8s_fusion",
            holdout,
            0.5 * pred_2s + 0.5 * pred_8s,
        )
    )

    concat_train_df = build_concat_case_table(
        train_case_df_2s=per_window_result["2s"]["train_case_df"],
        train_case_df_8s=per_window_result["8s"]["train_case_df"],
    )
    concat_eval_embedding = np.concatenate(
        [
            np.asarray(per_window_result["2s"]["eval_case_df"]["embedding"].iloc[0], dtype=float),
            np.asarray(per_window_result["8s"]["eval_case_df"]["embedding"].iloc[0], dtype=float),
        ]
    )
    concat_residual_pred, concat_neighbors = predict_embedding_knn(
        train_case_df=concat_train_df,
        eval_embedding=concat_eval_embedding,
        target_column="rpm_residual_oof",
        neighbor_tag="embedding_residual_knn4_concat_2s_8s",
        holdout_case_id=holdout.case_id,
    )
    concat_clip = max(float(concat_train_df["rpm_residual_oof"].abs().quantile(0.95)), 0.15)
    concat_residual_pred = float(np.clip(concat_residual_pred, -concat_clip, concat_clip))
    case_rows.append(
        build_case_row(
            domain_name,
            "rpm_knn4__plus__embedding_residual_knn4_concat_2s_8s_w0.5",
            holdout,
            base_pred + 0.5 * concat_residual_pred,
        )
    )
    for row in concat_neighbors:
        row["holdout_domain"] = domain_name
        neighbor_rows.append(row)

    avg_residual_2s_8s = 0.5 * (
        extract_variant_pred(case_rows, "rpm_knn4__plus__embedding_residual_knn4_2s_w0.5")
        + extract_variant_pred(case_rows, "rpm_knn4__plus__embedding_residual_knn4_8s_w0.5")
    )
    case_rows.append(
        build_case_row(
            domain_name,
            "rpm_knn4__plus__embedding_residual_knn4_avg_2s_8s_w0.5",
            holdout,
            avg_residual_2s_8s,
        )
    )
...
def build_concat_case_table(
    *,
    train_case_df_2s: pd.DataFrame,
    train_case_df_8s: pd.DataFrame,
) -> pd.DataFrame:
    merged = train_case_df_2s[
        ["case_id", "file_name", "true_wind_speed", "rpm", "rpm_pred_oof", "rpm_residual_oof", "embedding"]
    ].rename(columns={"embedding": "embedding_2s"}).merge(
        train_case_df_8s[["case_id", "embedding"]].rename(columns={"embedding": "embedding_8s"}),
        on="case_id",
        how="inner",
    )
    merged["embedding"] = merged.apply(
    ...
    return merged.drop(columns=["embedding_2s", "embedding_8s"]).reset_index(drop=True)
...
def predict_rpm_knn4(records: list[DatasetRecord], rpm_value: float | None) -> float:
    train_df = pd.DataFrame([{"rpm": float(record.rpm), "wind_speed": float(record.wind_speed)} for record in records])
...
def extract_variant_pred(case_rows: list[dict[str, object]], variant_name: str) -> float:
    for row in case_rows:
        if row["variant_name"] == variant_name:
            return float(row["pred_wind_speed"])
    raise KeyError(f"未找到变体: {variant_name}")
...
def write_summary_markdown(output_path: Path, summary_df: pd.DataFrame, case_level_df: pd.DataFrame) -> None:
    lines = [
        "# TinyTCN embedding 窗长信号 quickcheck",
        "",
