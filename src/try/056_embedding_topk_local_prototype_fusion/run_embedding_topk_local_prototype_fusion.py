...
TRY_NAME = "056_embedding_topk_local_prototype_fusion"
...
HEAD_VARIANTS = {
    "rpm_knn4__plus__embedding_topk_prototype_ridge": 1.0,
    "rpm_knn4__plus__embedding_topk_prototype_ridge_w0.5": 0.5,
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="embedding top-k local prototype fusion quickcheck。")
...
def main() -> None:
    ...
    print("056 embedding top-k local prototype fusion quickcheck 已完成。")
...
def build_prototype_feature_row(
    *,
    ...
    for index, value in enumerate(target_vec, start=1):
        feature_row[f"target_embed_{index}"] = float(value)
    for index, value in enumerate(ref_vec, start=1):
        feature_row[f"ref_embed_{index}"] = float(value)
...
def write_summary_markdown(output_path: Path, summary_df: pd.DataFrame, case_level_df: pd.DataFrame) -> None:
    lines = [
        "# embedding top-k local prototype fusion quickcheck",
        "",
    ...
    focus_variants = [
        ...
        "rpm_knn4__plus__embedding_topk_prototype_ridge",
    ]
