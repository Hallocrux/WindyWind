...
TRY_NAME = "029_postconv_channel_attention_tinytcn_quickcheck"
...
class HiddenChannelSE(nn.Module):
...
class TinyTCNBackbone(nn.Module):
...
class SingleStreamTinyTCN(nn.Module):
    def __init__(self, in_channels: int) -> None:
        super().__init__()
        self.backbone = TinyTCNBackbone(in_channels=in_channels, out_channels=32)
        self.pool = nn.AdaptiveAvgPool1d(1)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        feat = self.pool(feat_map).squeeze(-1)
        return self.head(feat).squeeze(-1)


class PostConvChannelAttentionTinyTCN(nn.Module):
    def __init__(self, in_channels: int) -> None:
        super().__init__()
        self.backbone = TinyTCNBackbone(in_channels=in_channels, out_channels=32)
        self.attn = HiddenChannelSE(channels=32, reduction=4)
        self.pool = nn.AdaptiveAvgPool1d(1)
        self.head = nn.Sequential(
            nn.Linear(32, 16),
            nn.ReLU(),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="快速验证后卷积通道注意力 TinyTCN。")
...
def main() -> None:
    ...
    target_case_df = evaluate_case_group(dataset, TARGET_CASE_IDS, train_config, args.random_seed)
    ...
    if attn_better:
        control_case_df = evaluate_case_group(dataset, CONTROL_CASE_IDS, train_config, args.random_seed)
        control_summary_df = summarize_case_df(control_case_df)
    else:
        control_case_df = pd.DataFrame(columns=["case_id", "file_name", "true_wind_speed", "pred_mean", "abs_error", "variant_name"])
        control_summary_df = pd.DataFrame(columns=["variant_name", "case_mae", "case_max_error", "improved_case_count"])
    ...
    print("后卷积通道注意力 TinyTCN 快速验证已完成。")
    print(f"输出目录: {output_dir}")
    print(f"目标工况: {TARGET_CASE_IDS}")
    print(f"是否继续了对照工况: {attn_better}")


def evaluate_case_group(dataset, case_ids: list[int], train_config: TrainConfig, random_seed: int) -> pd.DataFrame:
    ...
    for case_id in case_ids:
        ...
        for variant_name in ("SingleStreamTinyTCN", "PostConvChannelAttentionTinyTCN"):
            ...
            if variant_name == "SingleStreamTinyTCN":
                ...
                model = PostConvChannelAttentionTinyTCN(in_channels=X_train.shape[1]).to(device)
...
def summarize_case_df(case_df: pd.DataFrame) -> pd.DataFrame:
    baseline = case_df[case_df["variant_name"] == "SingleStreamTinyTCN"][["case_id", "abs_error"]].rename(columns={"abs_error": "baseline_abs_error"})
    ...
    return (
        ...
        .agg(case_mae=("abs_error", "mean"), case_max_error=("abs_error", "max"), improved_case_count=("improved_vs_baseline", "sum"))
...
def is_attention_positive(summary_df: pd.DataFrame, case_df: pd.DataFrame) -> bool:
    attn_mae = float(summary_df.loc[summary_df["variant_name"] == "PostConvChannelAttentionTinyTCN", "case_mae"].iloc[0])
    ...
    attn = case_df[case_df["variant_name"] == "PostConvChannelAttentionTinyTCN"][["case_id", "abs_error"]].rename(columns={"abs_error": "attn_abs_error"})
...
def write_summary_markdown(
    ...
    lines = [
        "# 后卷积通道注意力 TinyTCN 快速验证结论",
        "",
        f"- 目标难工况：`{TARGET_CASE_IDS}`",
        f"- 对照工况：`{CONTROL_CASE_IDS}`",
        "- 窗口口径：`50Hz / 5s / 2.5s`",
        "- 注意力位置：`post-conv hidden channels`",
        "",
        "## 目标难工况汇总",
        "",
    ]
    for _, row in target_summary_df.iterrows():
        lines.append(f"- `{row['variant_name']}`: case_mae=`{row['case_mae']:.4f}`, case_max_error=`{row['case_max_error']:.4f}`, improved_case_count=`{int(row['improved_case_count'])}`")
