...
- `src/Baseline_TinyTCN/`
  ...
  - `2026-04-10` 起已独立收口，不再依赖 `src/try/` 下的探索代码
- `src/modal_parameter_identification/`
  - `2026-04-09` 起生效的模态参数识别正式模块
- `src/try/`
  - 一次性探索目录，不与正式主线混放
...
### 5.34 [2026-04-09] added-first 子项目收尾后，当前默认最佳模型固定为 `071 | rpm_knn4 + embedding residual ridge`
...
- 证据入口：
  - `outputs/try/071_external_embedding_regression_quickcheck/summary_by_protocol.csv`
  - `outputs/try/079_repo_fft_sideinfo_in_071_residual/summary_by_protocol.csv`

当前 added-first 收尾结论显示：
...
- 当前 added-first 子项目的默认最佳模型应固定为 `071`；
- 把 repo FFT side-info 直接并入 `071` residual ridge，没有形成可升级的正信号；
- 因此该子项目当前应视为已完成收尾，后续若重启，应以 `071` 为默认对照位。

### 5.35 [2026-04-09] `final` 训练的 `acc_only` 版 `TinyTCN 2s+8s` 外推虽明显优于 `all_channels`，但仍不适合作为 `added2` 默认主线
...
- `added2 | acc_only_8s`
  - `case_mae = 1.7812`
- `added2 | acc_only_2s_8s_fusion`
  - `case_mae = 1.7897`
- `added2 | all_channels_2s_8s_fusion`
  - `case_mae = 2.2897`
- `added2 | rpm_knn4`
  - `case_mae = 0.8131`
...
- `all_channels` 的 direct learned 崩坏，当前确实主要由应变侧驱动；
- 把输入切到 `acc_only` 后，`2s / 8s / 2s+8s` 都明显更稳；
- 但它们仍然明显弱于 `rpm_knn4`；
- 并且在外部域上，`2s+8s` 也没有优于单独 `8s`，因此当前不能升级为 `added2` 默认 direct learned 主线。

### 5.35 [2026-04-09] 模态参数识别已从 `try/065` 正式沉淀到 `src/modal_parameter_identification/`
...
- 默认输出目录：
  - `outputs/modal_parameter_identification/`
- 当前已正式提供：
  - `PSD / CSD / coherence / FDD / SSI-COV`
  - `strain / acc_y` 两套 `5` 测点离散振型
  - 显式触发的 `1D` 塔身折线振型动画导出

这说明：

- 模态参数识别这条线不再继续以 `src/try/065_modal_parameter_identification/` 作为默认主入口；
- `src/try/065_modal_parameter_identification/` 当前仅保留历史代码与兼容入口；
- `2026-04-09` 起，用户侧和后续开发都应优先基于正式模块与正式输出目录继续迭代。

### 5.36 [2026-04-12] 不用风速监督的 contrastive condition embedding V1 已跑通，但仍未超过 `071`
...
当前 `added_to_added2` quickcheck 结果显示：
...
- `contrastive_embedding_ridge`
  - `case_mae = 1.6242`
- 对照 `071 | rpm_knn4 + embedding residual ridge`
  - `case_mae = 0.6161`

这说明：

- 不直接使用风速监督、按 `case_id` 对比学习训练出的工况表征已经能提供 residual 修正信号；
- 但当前最小版仍弱于 `071` 的风速监督 embedding residual；
- `086` 当前应保留为研究型表征候选，不应替代 `071` 成为 added-first 默认最佳模型。

### 5.37 [2026-04-13] `L_reg + lambda_ssl * L_ssl` 联合训练 V1 优于 contrastive-only V1，但仍未超过 `071`
...
- `087 | rpm_knn4 + joint_embedding_residual_ridge`
  - `case_mae = 0.8935`
...
- 在当前 `lambda_ssl = 0.1` 口径下，加入风速回归项后，联合训练 embedding 比 contrastive-only embedding 更适合 residual 修正；
- 但 `087` 仍明显弱于 `071`；
- 当前默认 added-first 最佳模型仍应保持 `071 | rpm_knn4 + embedding residual ridge`。

### 5.38 [2026-04-13] `071` residual regressor 替换实验未支持替换默认 Ridge
...
当前 `added_to_added2` 主测试结果显示：

- `rpm_knn4_plus_residual_svr_linear`
  - `case_mae = 0.6089`
  - 相比 `071` 的 `case_mae = 0.6161` 改善 `0.0072`
- `rpm_knn4_plus_residual_ridge_cv`
  - `case_mae = 0.6161`
  - 即 `071` 对照口径
- `rpm_knn4_plus_residual_pls3`
  - `case_mae = 0.6161`
- `rpm_knn4_plus_residual_kernel_ridge_linear`
  - `case_mae = 0.6318`
- `rpm_knn4_plus_residual_elasticnet_cv`
  - `case_mae = 0.6359`
- `rpm_knn4`
  - `case_mae = 1.2903`

次级 `external_loocv` 复核显示：
...
- linear SVR 在 `added_to_added2` 主测试中只比 `071` ridge 微弱改善，优势来自 `工况26`、`工况27`、`工况30`，但在 `工况25`、`工况28`、`工况29` 弱于 ridge；
- linear SVR 的跨协议表现明显不稳，不适合作为 `071` 的默认替换；
- `PLS3` 与 ridge 几乎等价，其余候选未在主测试中超过 `071`；
- `2026-04-13` 起，默认 added-first 最佳模型仍应保持 `071 | rpm_knn4 + embedding residual ridge`。

### 5.39 [2026-04-13] non-causal dilated CNN encoder 在 `added_to_added2` 上超过 `071`，但仍需复核
...
- `noncausal_dilated | rpm_knn4_plus_embedding_residual_ridge`
  - `case_mae = 0.5270`
- `071 | rpm_knn4_plus_embedding_residual_ridge`
  - `case_mae = 0.6161`
- `noncausal_nodilation | rpm_knn4_plus_embedding_residual_ridge`
  - `case_mae = 1.2563`
- `rpm_knn4`
  - `case_mae = 1.2903`
...
- 去掉 causal crop 并保留 dilation 后，`added_to_added2` 主测试从 `071` 的 `0.6161` 改善到 `0.5270`；
- 去掉 dilation 后，表现退回到接近 `rpm_knn4` 的水平，说明本轮收益更可能来自 non-causal 对称上下文、dilation 感受野和 residual block 的组合；
- 由于 `external_loocv` 未超过纯 `rpm_knn4`，`2026-04-13` 不直接替换最终默认模型，但 `noncausal_dilated` 是下一步 added-first 复核的优先候选。
- `2026-04-13` 已对 `noncausal_dilated` 的全部 `30` 个工况、`64` 维 case embedding 做标准化 PCA，前两维解释方差为 `PC1 = 59.60%`、`PC2 = 26.13%`。
...
- added-first 策略与最终对齐：`Docs/added_first_strategy_and_alignment_2026-04-09.md`
- added-first 阶段性工作假设：`Docs/added_first_working_hypothesis_2026-04-09.md`
...
- final 训练的 `acc_only` 版 `2s+8s` 晚融合外推 added2：`Docs/experiments/final_late_fusion_acc_only_added2_replay_2026-04-09.md`
- contrastive condition embedding quickcheck：`Docs/experiments/contrastive_condition_embedding_quickcheck_2026-04-12.md`
- joint reg ssl condition embedding quickcheck：`Docs/experiments/joint_reg_ssl_condition_embedding_quickcheck_2026-04-13.md`
- `071` residual regressor ablation：`Docs/experiments/071_residual_regressor_ablation_2026-04-13.md`
- non-causal CNN encoder ablation：`Docs/experiments/noncausal_cnn_encoder_ablation_2026-04-13.md`
- added+added2 训练池的 `TinyTCN 2s+8s` 少量 LOCO quickcheck：`Docs/experiments/added_added2_late_fusion_loco_quickcheck_2026-04-09.md`
- added SOTA 回放 added2：`Docs/experiments/added_sota_added2_replay_2026-04-08.md`
- repo FFT side-info 并入 071 residual quickcheck：`Docs/experiments/repo_fft_sideinfo_in_071_residual_2026-04-09.md`
- 线性表格基线 added-first 补测：`Docs/experiments/tabular_linear_baselines_added_first_eval_2026-04-09.md`
